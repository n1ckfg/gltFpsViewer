...
- **Post-processing**: When the levels are not neutral the scene is drawn through an `EffectComposer` chain — `RenderPass` → `OutputPass` → a `ShaderPass` running `LevelsShader`. Because the levels pass runs *after* `OutputPass` has tone-mapped and sRGB-encoded the frame, it operates on display-referred values, matching the behaviour of an image editor's Levels dialog.
- **Composer Bypass**: Neutral levels (0 / 1 / 1, the default) make the chain a pass-through, so `setLevels()` flips a flag and `animate()` calls `renderer.render()` directly instead. That removes two full-screen passes and the half-float buffers from the common case, and restores the renderer's own MSAA. The two paths are pixel-identical — colours round-trip exactly through either. The composer's render targets are given `samples = 4` so that switching to it does not silently drop antialiasing.
...
- **Widget Visibility**: `updateWidgetVisibility()` hides everything that is viewer chrome rather than scene content whenever the pointer is locked — the `GridHelper`, the `TransformControls` gizmo and the green selection `BoxHelper` — so flight mode (and anything captured during it) is unobstructed. Photos hide the same widgets in either mode (see section 5). Selection state is kept, not cleared: the gizmo and box reappear on the same node when the menu comes back. Note that `PointerLockControls` dispatches its `lock`/`unlock` events *before* assigning `isLocked`, so those two handlers pass the new state in explicitly rather than reading the stale flag.
...
- **Controls**: `R` arms a 3-second countdown (rendered as a large centred number in the `#recorder-hud` overlay), after which capture begins. `R` again stops the recording and triggers the download; pressing it *during* the countdown cancels instead. The hotkey works in both FPS Flight and Object Manipulation modes.
- **Capture Path**: Two paths, chosen when a take starts. If the renderer's drawing buffer already fits within the capture bounds, `MediaRecorder` captures the WebGL canvas **directly** — no per-frame copy at all. If it is larger, each frame is blitted into a fixed-size offscreen 2D canvas immediately after the render (while the drawing buffer is still valid, so `preserveDrawingBuffer` is not required) and that canvas is captured instead, scaling the frame down and letterboxing it. The blit is a GPU readback, so it is paced to the capture frame rate via `captureStream(0)` plus `track.requestFrame()` — one readback per encoded frame rather than one per animation frame — falling back to a timed `captureStream(fps)` where `requestFrame` is unavailable. Locking the scaled output resolution at the start of a take means a mid-recording resize letterboxes rather than breaking the stream; in direct mode a resize changes the stream's resolution instead, and is warned about on the console.
...
- **Capture Render Scale**: While a take is armed or running, `applyCaptureRenderScale()` in `main.js` lowers the renderer's pixel ratio so the drawing buffer lands on the capture bounds rather than the display's full device pixel ratio. The encoded resolution is unchanged — those pixels were being scaled away anyway — but far fewer are drawn, and the capture can then take the direct path. On a HiDPI display this roughly doubles the captured frame rate; the tradeoff is that the image on screen is rendered at that lower ratio for the duration of the take, and the video no longer gets the supersampling the downscale used to provide. Set `MATCH_RENDER_SCALE_TO_CAPTURE` to `false` to always render at full device resolution.
...
### 5. Photo Capture
- **Controls**: `Space` saves the current frame as a timestamped `photo_*.png`, in both FPS Flight and Object Manipulation modes. Key repeat is ignored, so holding the key takes one photo.
- **Capture Path**: The key handler only sets a flag; the read happens in `animate()` right after the render, while the drawing buffer is still valid (the same trick the recorder uses, so `preserveDrawingBuffer` is still not required). `canvas.toBlob()` copies the pixels synchronously and encodes the PNG asynchronously.
- **Widgets Hidden**: Photos never show viewer chrome (grid, gizmo, selection box), even in Object Manipulation Mode. On a photo frame `animate()` hides the widgets via `updateWidgetVisibility( true )`, renders, reads the canvas, restores them, then does its normal render. That second render overwrites the first before the browser composites, so the widgets never blink off on screen or in a concurrent video; the cost is one extra render per photo.
- **What's in the Image**: The scene as displayed, background colour and levels included. The image is at the renderer's drawing-buffer resolution, so a photo taken while a take is armed or running comes out at the reduced capture render scale.
- **Flash**: A full-screen white `#flash` overlay fades out over 500ms via the Web Animations API. Like the recorder HUD it is plain DOM over the canvas, so it never appears in the PNG — or in a video being recorded at the same time.

### 6. Colour Palette & Levels (`palette.js`, `levels.js`)
