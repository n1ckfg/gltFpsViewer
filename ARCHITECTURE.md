...
- **What's in the Image**: Exactly what the canvas shows — background colour and levels included, and the viewer widgets too if they are visible (i.e. in Object Manipulation Mode). The image is at the renderer's drawing-buffer resolution, so a photo taken while a take is armed or running comes out at the reduced capture render scale.
